...
class Material:
    ...
    def _createSurfaceShader(self, usdMaterial, usdStage):
        ...
        usdMaterial.CreateOutput('surface', Sdf.ValueTypeNames.Token).ConnectToSource(UsdShade.ConnectableAPI(surfaceShader), 'surface')
    ...
    def _makeUsdUVTexture(self, matPath, map, inputName, channels, uvInput, usdStage):
        ...
        if uvReader:
            ...
            if uvInput != None:
                ...
                uvReader.CreateInput('varname', Sdf.ValueTypeNames.Token).ConnectToSource(UsdShade.ConnectableAPI(uvInput))
            else:
        ...
        if map.transform != None:
            ...
            transformShader.CreateInput('in', Sdf.ValueTypeNames.Float2).ConnectToSource(UsdShade.ConnectableAPI(uvReader), 'result')
        ...
        textureShader.CreateInput('st', Sdf.ValueTypeNames.Float2).ConnectToSource(UsdShade.ConnectableAPI(uvReader), 'result')
    ...
    def _addMapToUsdMaterial(self, inputIdx, usdMaterial, surfaceShader, usdStage):
        ...
        if isinstance(input, Map):
            ...
            surfaceShader.CreateInput(inputName, inputType).ConnectToSource(UsdShade.ConnectableAPI(textureShader), channels)
